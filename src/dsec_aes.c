                         ...
                         uint32_t iv_size)
{
    ...
    if ((tag != NULL) &&
        ...
        operation.params[0].tmpref.buffer = tag;
        operation.params[0].tmpref.size = *tag_size;

        operation.params[1].tmpref.buffer = key_data;
        operation.params[1].tmpref.size = key_data_size;

        operation.params[2].tmpref.buffer = data_in;
        operation.params[2].tmpref.size = data_in_size;
        ...
        if (result == DSEC_SUCCESS) {
            *tag_size = operation.params[0].tmpref.size;
        } else {
            *tag_size = 0;
            (void)dsec_print("An error occurred: TEEC_Result=0x%x, "
                             "DSEC_E=0x%x\n",
